...
void onTick(CBlob@ this)
{
	...
	if (ball !is null)
	{
		if (data.local)
		{
			...
			CBlob@[] ol;
			if (this.getOverlapping(@ol))
			{
				float myabs_x = Maths::Abs(this.getVelocity().x);
				for (u32 i = 0; i < ol.length; i++)
				{
					CBlob@ overlapped = ol[i];
					if (overlapped.getName() == "soldier" &&
					        overlapped.isKeyPressed(key_crouch) &&
					        Maths::Abs(overlapped.getVelocity().x) > myabs_x)
					{
						force_drop = true;
						break;
					}
			...
			if (force_drop || this.isKeyJustPressed(key_action2))
			{
				f32 drop_speed = 1.5f;
